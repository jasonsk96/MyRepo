...
public class TwitterController {
	...
	TwitterController(TweetRepository repository, TweetModelAssembler tweetAssembler) {
	...
	// Aggregate root
	  ...
	  @GetMapping("/tweets")
	  CollectionModel<EntityModel<Tweet>> all() {

	    List<EntityModel<Tweet>> tweets = repository.findAll().stream() //
	        .map(tweetAssembler::toModel) //
	        .collect(Collectors.toList());
	    
	    return CollectionModel.of(tweets, WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(TwitterController.class).all()).withSelfRel());
	  }
	  ...
	  ResponseEntity<?> configure(@RequestBody Configurations config) {
			  if(twitterRetrieval)
				  return ResponseEntity.ok("You can configure the SMA adapter before the Call [Get-method begin].");
	  ...
	  @PostMapping("/tweets")
	  ResponseEntity<?> newTweet(@RequestBody Tweet newTweet) {

	    EntityModel<Tweet> entityModel = tweetAssembler.toModel(repository.save(newTweet));
	...
	// Single item
	  ...
	  ResponseEntity<?> begin() {
		  if(twitterRetrieval)
			    return ResponseEntity.ok("[Get-method begin] has already been called.");
					t1 = new Thread(new ConfigNRetrieve (repository));
					  t1.start();  
