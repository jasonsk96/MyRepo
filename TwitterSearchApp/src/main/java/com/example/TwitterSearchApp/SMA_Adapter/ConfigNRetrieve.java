...
public class ConfigNRetrieve implements Runnable{
	...
	ConfigNRetrieve(TweetRepository repository) {
	...
	public  void run(){
		...
		for(int i=0; i<mytweets.size(); i++){
			tweetRepository.save(new Tweet(mytweets.get(i)));
			//tweets.add(mytweets.get(i));
		}
		//tweets = result.getTweets();
	    twitersIDs.add((float) mytweets.get(0).getId());
	    int keyword_index=0;
	    ...
	    while(oldTweetsExist) { //We are in the case of retrieving old tweets.
		 ...
		 System.out.println("Old Tweets: We have "+tweetRepository.count()+" tweets."); 	
	    ...
	    while(true) { //We are in the case of waiting for new tweets.
		 ...
		 System.out.println("New Tweets: We have "+tweetRepository.count()+" tweets.");   
		}
