...
import com.example.TwitterSearchApp.SMA_Adapter.TweetResult;
import com.example.TwitterSearchApp.SMA_Adapter.TwitterController;
import com.google.gson.Gson;
import com.example.TwitterSearchApp.SMA_Adapter.Search;
...
public class OntController {
	...
	public ResponseEntity<?> search(@RequestBody InputClass input) throws Exception{
		...
		if(input.items.isEmpty() && input.mykeywords.replace(",", "").trim().isEmpty()) {
			...
			ResponseEntity<String> response = restTemplate.postForEntity("http://"+prop.getProperty("domain")+":"+prop.getProperty("port")+"/retrieve", request, String.class);
